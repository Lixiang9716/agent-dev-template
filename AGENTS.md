...
3. **Prove each gate rejects an invalid case.** A guard only guards if the regression actually fails it. Every verifier ships with tests that demonstrate rejection; `scripts/gates.test.mjs` is the pattern.
...
7. **Bilingual pairs merge whole.** A PR never lands one language of a pair without the other two files; re-confirm with `node scripts/verify-translation-pairing.mjs --write <path>` in the same change.
...
9. **Select the smallest sufficient check set.** Run `node scripts/change-scope.mjs --base <verified-ref>` and pick gates by touched surface; never reflexively run the full aggregate. CI owns exhaustiveness.
