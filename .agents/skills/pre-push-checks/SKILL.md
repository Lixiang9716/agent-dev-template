...
3. From the reported path classes, pick gates:
   - anything under `scripts/` or `gates.json` → `self-test`, plus `script-pairs` when a twin script changed (re-confirm with `verify-script-pairs --write` in the same change)
   - anything under `.agents/notes/` → `notes`, and `archive` when `archived/` changed
   - any `.md`/`.zh.md`/`.i18n.yaml` → `pairing`, `budgets` when ceilings or budgeted docs changed, `links` when Markdown targets or headings changed
