...
模板只提供治理平面。它不预设你的编程语言、测试框架或包管理器——你的工具链以命令槽位的形式接入 `gates.json`。每个脚本提供两份等价实现:bash 版(`scripts/*.sh`,要求 bash 5+)与 PowerShell 版(`scripts/*.ps1`,要求 pwsh 7+)。主机上有什么就用什么,无额外运行时、无安装步骤。成对探针在对方解释器存在时运行、缺失时响亮跳过;CI 强制探针。
