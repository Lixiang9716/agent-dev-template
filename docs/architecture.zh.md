...
每个治理脚本提供两份等价实现:bash 版(`scripts/*.sh`,兼容 macOS 的 bash 3.2)与 PowerShell 版(`scripts/*.ps1`,要求 pwsh 7+)。两者读取同一个 `gates.json`,产出同一套词汇表;CI 两者都跑。孪生配对一起确认:`scripts/script-pairs.json` 钉住两侧的 blob 哈希,漂移的配对会让门禁变红,直到同一改动内用 `--write` 重新确认——重确认就是"孪生文件已被考虑"的显式凭证。配对可以声明行为探针:运行两侧的测试套件并比较归一化后的输出;探针具备可用性感知——对方解释器在 PATH 上才运行,不在则响亮跳过,因此只装 bash 或只装 pwsh 的主机也能通过全部本地门禁,而 CI 在每条矩阵腿上强制探针(`GATES_FORCE_PROBE=1`;其闭合集是 {unset, 1}——其他值点名 fail loud)。门禁槽位是纯命令数组(两种 shell 下相同)或按 shell 的变体;变体对象必须写全封闭集合里的每种 shell——缺变体直接中止,而不是在那个平台上静默跳过。
...
并发默认取 CPU 数,可用 `GATE_CONCURRENCY` 封顶。输出按门禁捕获:通过的门禁保持静默(设 `GATE_VERBOSE=1` 可见),唯独响亮跳过行即使在通过的门禁上也会浮出——被跳过的探针绝不能看起来像完整覆盖;失败的门禁打印命令、结果与输出。
